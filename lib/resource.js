...
var Resource = function (filename, cb) {
  ...
  fs.readFile(self.filename, function (err, data) {
    ...
    self.processinit(function () {
      ...
      if (self.parts.length == 0 && !self.cache) {
        self.cache = function (hash, callback) {
      ...
      var end = function () {
        ...
        if (i >= resource.parts.length) {
          ...
          resource.cacheResponse = 
            ...
            , buffer: new Buffer(responseString)
            , hash: hash
            }
          resource.respond(req, resp);
        }
...
var HtmlResource = function (filename, cb) {
  Resource.apply(this, [filename, cb]);
...
exports.createResource = function (filename, callback) {
  new (extmap[path.extname(filename)] || extmap.default)(filename, callback)
}
